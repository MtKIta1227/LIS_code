...
class CyclePlotterWidget(QWidget):
    ...
    def plot_range_cycles(self):
        # Deselect and select only visible items matching the input cycle range
        ...
        # Get cycle range from user
        text, ok = QInputDialog.getText(
            self, "Input Cycle Range",
            "Enter cycle numbers (e.g. 1-3,6,8):",
            text="1,2,10,30,50,100,200,300"
        )
        if not ok or not text:
            return

        # Parse the range string into a set of cycle numbers
        ...
        # Select only visible items matching the cycle set
        valid_ids = []
        for i in range(self.list_widget.count()):
            item = self.list_widget.item(i)
            # Skip items hidden by filter
            if item.isHidden():
                continue
            data = item.data(Qt.UserRole)
            if data:
    ...
    def plot_dis_cap_efficiency(self):
        ...
        select_dialog.setWindowTitle("Plot Efficiency: Select Samples")
        dlg_layout = QVBoxLayout(select_dialog)
        sample_list = QListWidget()
        sample_list.setSelectionMode(QListWidget.NoSelection)
        for s in sorted(self.data_by_sample.keys()):
            item = QListWidgetItem(s)
            item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
            item.setCheckState(Qt.Unchecked)
            sample_list.addItem(item)
        dlg_layout.addWidget(sample_list)
        btns = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        btns.accepted.connect(select_dialog.accept)
        btns.rejected.connect(select_dialog.reject)
        dlg_layout.addWidget(btns)
        if not select_dialog.exec_():
            return
        selected = [sample_list.item(i).text() for i in range(sample_list.count())
                    if sample_list.item(i).checkState() == Qt.Checked]
        if not selected:
            QMessageBox.warning(self, "No selection", "Please select at least one sample.")
            return
        ...
        if multiple:
            ...
            for i, cyc in enumerate(cycles):
                alpha = 1.0 if self.mono_mode else 1.0 - 0.1 * ((cyc - 1) % 10)
                color = "black" if self.mono_mode else cmap_used(((cyc - 1) % 10) % cmap_used.N)
                self.ax.plot([cyc], [max_dis_caps[i]], color=color, marker='o', label="DIS capacity" if i==0 else "")
                self.ax2.plot([cyc], [efficiencies[i] * 100], color=color, marker='x', label="Coulombic eff." if i==0 else "")
